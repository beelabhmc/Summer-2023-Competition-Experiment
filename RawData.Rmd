...
pw399 %>% filter(caste %in% c("workers", "soldiers"), nest != "base") %>%
  ...
  geom_image(aes(image=ifelse(queentf,"queen_crown_transparent.png", NA)), data = . %>% filter(caste %in% c("soldiers")), position = position_dodgenudge(y = 0.1, direction = "split", width = 0.75))+
