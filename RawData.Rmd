...
```{r}
#nest_pop = colSums(workers + soldiers) need to get the nest pop for each colony
...
```{r}
ants_wide <- ants_wide %>% 
  mutate(ants__nest_prop = (workers+soldiers)/pop_size, workers_prop = (workers)/pop_size, soldiers_prop = (soldiers)/pop_size )
```
...
```{r}
ants_wide %>% ggplot(aes(x=nest, y=Colony, color = diff_in_prop)) + geom_point(size=5) + scale_color_gradient2(low="mediumpurple",mid="white",high="darkgoldenrod1", midpoint = median(diff_in_prop), guide=guide_colorbar(direction="horizontal",title.position="top"))
```
